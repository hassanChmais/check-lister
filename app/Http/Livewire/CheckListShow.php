...
class CheckListShow extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.check-list-show');
    }
    ...
    public function complete_task($task_id){
        $task = Task::find($task_id);
       if($task){
                $user_task = Task::where('task_id', $task_id)->where('user_id',auth()->id())
                ->first();
                if($user_task){
                if (is_null($user_task->completed_at)) {
                    ...
                    $this->completed_tasks = array_diff($this->completed_tasks,[$task_id]);
                }
                }
        else{
